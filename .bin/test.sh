...
if [[ -n $match_hour ]] && [[ -n $match_teams ]]; then
  touch "/tmp/info_displayed"
  info_displayed=$(cat "/tmp/info_displayed")
  ...
  if [[ $info_displayed == "match" ]]; then
    echo "displaying match"
    echo "$match_teams" >"$santoss_match"
    echo "hour" >"/tmp/info_displayed"
  elif [ $info_displayed == "hour" ]; then
    echo "displaying hour"
    echo "$match_hour" >"$santoss_match"
    echo "match" >"/tmp/info_displayed"
  fi
