...
# Prompt user to enter a file name using wofi
file_name=$(find "$search_dir" -maxdepth 1 -type f | sed "s|$search_dir/||" | wofi --dmenu --prompt "")
...
if [ -e "$full_path" ]; then
    ...
    foot -T 'maps' -a "h-m-m" -e h-m-m "$full_path"
else
