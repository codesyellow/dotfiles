...
const santosMatch = async (hour, match) => {
    ...
    while (true) {
        ...
        if (timeLeft <= 0) {
            ...
            if (fs.existsSync("/tmp/santosmatch")) {
        ...
        if (timeLeft <= 2 && gameMinutes <= current_minute && !notifyDone) {
            await $`notify-send.sh -u critical -i $HOME/.local/share/icons/football.png "Pregame is about to start!!!"`;
            notifyDone = true;
        }
...
const isMatchToday = () => {
    const matchs = jsonSantosCalendar
    ...
    if (!willSantosPlay) {
        console.log(`Santos will not be playing today.`);
    }
};
