...
groups = [
        Group(icons[0], layout='max', matches=[Match(wm_class=re.compile(r"^(navigator|firefox|Brave\-browser|qutebrowser|org\.qutebrowser\.qutebrowser|floorp)$"))]),
        Group(icons[1], layout='max', matches=[Match(wm_class=re.compile(r"^(audacious)$"))]),
        Group(icons[2], layout='max', matches=[Match(wm_class=re.compile(r"^(Alacritty|st)$"))]),
