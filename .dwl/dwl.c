...
{
    wl_list_remove(&c->link);       // Remove from the current position
    wl_list_insert(&clients, &c->link); // Insert at the top of the stack
}
...
togglescratch(const Arg *arg)
{
    ...
    // Search for the first window that matches the scratchkey
    wl_list_for_each(c, &clients, link)
        if (c->scratchkey == ((char**)arg->v)[0][0]) {
            found = 1;
            break;
    ...
    if (found) {
        if (VISIBLEON(c, selmon)) {
            ...
            // Show and force it to be on top
            c->tags = selmon->tagset[selmon->seltags];
            c->isfloating = 1;  // Ensure it's floating
            c->isfullscreen = 0; // Disable fullscreen to ensure it shows on top
            focusclient(c, 1);
            raiseclient(c); // Force the window to the top of the stack
        }
        arrange(selmon);
    } else {
        spawnscratch(arg);
